 ...
 * 19. countdown_left
 * 20. total_pm
 * 21. air_quality (on Morento HY4866-WF)
...
class AirPurifierAccessory extends BaseAccessory {
         ...
         if (this.device.context.noRotationSpeed) {
            ...
            switch (this.device.context.manufacturer) {
                 case 'Breville':
                     ...
                     break;
                 case 'Morento':
                      _fanSpeedLabels = {0: 'sleep', 1: '1F', 2: '2F', 3: '3F', 4: '4F', 5: 'auto'};
                      this._rotationSteps = [...Array(6).keys()];
                      fanSpeedSteps = 5;
                     break;
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            this.log.debug('Changes: %o, State: %o', changes, state);
             ...
             if (changes.hasOwnProperty(DP_FAN_SPEED)) {
                 ...
                 // Morento "auto" fan speed is not mapped and should not trigger a rotation speed update
                 if (newRotationSpeed) {
     ...
     /* Morento air purifier does not support DP_MODE but has fan speed 'auto' */
     _getMode(state) {
         if (state[DP_MODE]) {
    ...
    setTargetAirPurifierState(value, callback) {
        ...
        switch (value) {
             case Characteristic.TargetAirPurifierState.MANUAL:
                if (this.device.context.manufacturer == 'Morento') {
